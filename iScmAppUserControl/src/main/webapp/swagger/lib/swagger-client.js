    ...
    on: {
      error: function(response) {
        if (self.url.substring(0, 4) !== 'http')
          ...
          return self.fail('Can\'t read from service.  It may not have the appropriate access-control-origin settings.');
  ...
  for(path in response.paths) {
    if(typeof response.paths[path] === 'object') {
      ...
      for(httpMethod in response.paths[path]) {
        ...
        // bind this operation's execute command to the restapi
        if(tags.length > 0) {
          var i;
...
Operation.prototype.setContentTypes = function(args, opts) {
  ...
  if (consumes && this.consumes) {
    if (this.consumes.indexOf(consumes) === -1) {
      log('service doesn\'t consume ' + consumes + ', try ' + JSON.stringify(this.consumes));
    }
  }
  ...
  if (accepts && this.produces) {
    if (this.produces.indexOf(accepts) === -1) {
      log('service can\'t produce ' + accepts);
    }
  }
  ...
  if ((resourceObj.apis) && this.api.resourcePath) {
    ...
    var obj = {
      ...
      on: {
        ...
        error: function (response) {
          _this.api.resourceCount += 1;
          return _this.api.fail('Unable to read restapi \'' +
          _this.name + '\' from path ' + _this.url + ' (service returned ' + response.statusText + ')');
        }
      }
    };
    var e = typeof window !== 'undefined' ? window : exports;
...
SwaggerRequest.prototype.setHeaders = function (params, opts, operation) {
  ...
  if (consumes && this.operation.consumes) {
    if (this.operation.consumes.indexOf(consumes) === -1) {
      log('service doesn\'t consume ' + consumes + ', try ' + JSON.stringify(this.operation.consumes));
    }
  }
